...
As for the fellas, sections of this are still highly relevant, but obviously there are key differences in goals and outcomes. \href{https://mega.nz/folder/cfwSmBbY#ify6kaueB8SdDD1v12t2Dw}{This guide for masculinizing HRT} looks pretty solid, but I haven’t examined it in full depth, so use your brain and your judgement. Anyway they should make a tboy Katie Tightpussy. Oliver Longdick or something. Maybe Xavier.
...
\subsection{¿Es seguro consumir estrógeno?}

Con las hormonas bioidénticas modernas, el TRP difícilmente podría ser más seguro. Simplemente inviertes el combistible principal que usa tu cuerpo y cambias el balance hormonal que ya existe en tu cuerpo. Incluso cuando los detalles optimización se vuelven complejos, el principio central detrás de cambiar tu biología es bastante permisivo. El cuerpo es maleable y serás capaz de ajustarte a lo que sientas que es adecuado para ti.

\subsection{¿Qué forma de administración de estrógeno debería elegir?}

Inyecciones. Son, en general, la forma más a prueba de error, fácil, consistente, segura y barata de TRP. Para algunas, las inyecciones se vuelven un ritual esperado, para otras puede volverse bastante divertido.

\noindent\underline{\textbf{Recuerda: cualquier cantidad de estrógeno es mejor que ningún estrógeno.}}

\subsection{¿Por qué no recomiendas píldoras, parches o gel?}

Principalmente las tres tienen desventajas que las inyecciones no. No es que no sirvan, es que mereces más que ser forzada a tolerar desventajas no menores. Déjame reiterar: \textbf{todas las formas de TRP pueden producir resultados satisfactorios}, pero eso no significa que sern igual de buenas.

\subsection{¿Son las dosis de estrógeno equivalentes entre diferentes formas de aplicación??}

No. Esto es suficientemente importante para que no lo haya relegado a la sección \ref{MM} “MITOS Y MISC”.
\textbf{Las dosis de estrógeno no pueden ser comparadas de forma directa entre tipos y forma de aplicación.} 1mg de una no equivale a 1mg de otra. Los diferentes tipos y formas tienen diferentes propiedades que afectan cuánto estrógeno es absorbido por tu cuerpo (\textit{“biodisponibilidad”}), en que ratio y su vida media resultante.

\subsection{¿Qué es la “vida media”?}

De forma simple, la \textit{vida media} de una sustancia es el tiempo que tarda en eliminarse la mitad de ella. 
En el contexto del TRH, esto es lo que determina cuánto tiempo se mantiene una dosis activa en tu sistema, y por ende la frecuencia de dichas dosis. A esto se refiere como ciclo hormonal, que forma una curva. Los niveles suben, llegan a una cima y luego caen. Las propiedades de ésta curva (cómo los niveles de estrógeno cambian con el tiempo) son importantes.

\subsection{¿Qué hay de malo con las píldoras??}
...
Por favor, comprende que el riesgo absoluto es de todas formas bajo (p.e., el \textit{paracetamol} tiene un orden de magnitud mayor con respecto al hígado que el estrógeno), sin embargo \textbf{este riesgo se agrava con riesgos de estrógeno relacionados a la nicotina.} Ver preguntas \ref{11-2} también.

Más allá de esto, numerosos otros riesgos se derivan de las píldoras por dos características principales: 1) su corta vida media y biodisponibilidad pobre, y 2) su necesidad común de antiandrógenos. La primera característica hace que las píldoras sean mayoritariamente no aptas para monoterapia (discutído luego) comparadas con las inyecciones. La segunda viene acompañada por diversos efectos secundarios negativos, dependiendo de los antiandrógenos involucrados (ver sección \ref{AA} “ANTIANDRÓGENOS”).  Juntas, estas características suman adicionales grados de variabilidad que generan regímenes pobres, y sus efectos secundarios (como poca energía/libido y resultados más lentos) más comunes que con otros tipos de administración. Las píldoras son además más dificil de acaparar, y, en algunos mercados, son más caras que los frascos. Ten en cuenta que además, importar píldoras extranjeras en grandes volúmenes puede levantar alarmas aduaneras, lo que puede generar confiscaciones, pérdida monetaria y/o problemas legales, dependiendo de las leyes del país en que residas. \textbf{Si alguien pregunta, no sabes quien ordenó esas píldoras.}

\textbf{Si por alguna razón consumes el estrógeno en píldoras, por favor toma 4-8mg sublingualmente de forma espaciada durante el día.} Por debajo de 4mg no es casi nunca una dosis suficiente.

\subsection{¿Qué hay de malo con los parches?}

\begin{itemize}
  \item Relativamente caros (típicamente incluso más que las píldoras);
  \item Más dificil de conseguir DIY (sólo a través de mercado gris);
  ...
  \item Son difíciles de acaparar (difíciles de adquirir de forma mayorista);
  \item Generalmente dificiles de excederse de niveles de menopausia incluso con múltiples adheridos en simultaneo.
\end{itemize}

\subsection{¿Qué hay de malo con el gel?}

\begin{itemize}
  \item Difícil de dosificar de forma que de consistentemente genere los mismos niveles;
  \item Requiere una aplicación regular debido a su relativamente corta vida media;
  \item Puede ser un poco un desastre (viscoso);
  \item Riesgo de exposición de segundo grado debido al contacto con otra gente
  \item Generalmente necesita un antiandrógeno (ver sección \ref{AA} “ANTIANDRÓGENOS”).
\end{itemize}

Cabe señalas que el gel requiere mínimos insumos para la producción, lo cual puede ser una bendición en algunas circunstancias.

\subsection{¿Qué pasa con los pellets?}

\begin{itemize}
  \item Generalmente mucho más caro que cualquier otra opción;
  \item Pcos proveedores los ofrecen;
  \item Los períodos de ajuste de dosis están muy diseminads temporalmente;
  \item Los pellets fallidos pueden generar niveles insuficientes;
  \item Pellets aplastados/rotos pueden causar niveles altos inesperados;
  \item Generalmente no es posible de hacer de forma casea.
\end{itemize}

Particularmente el ultimo punto en particular significa que sólo podes comprar de los, probablemente, proveedores caros. Es posible que esta sea la primera vez que escuchas de los pellets. ¿Se ve el problema?

\subsection{¿Qué pasa con los aerosoles/vaporizadores?}

Son bastante experimentales por lo que no hay mucho para decir de ellos, pero comparten los pros y contras del gel. Esta sección es principalmente para que puedas saber de su existencia.

\subsection{¿Es tan significante la diferencia?}

\textbf{Sí.} Al punto que escribí todo esto para redirigir a esto en vez de tener que repetirme menos. Un régimen de inyecciones bien dosificado es la mejor forma de estrógeno que tenemos para los niveles en monoterapia. 
...
\textbf{No.} Mientras que esta guía no está interesada en responder individualmente a cada pedazo de desinformación de redes sociales, la prevalencia de esta tabla tanto online como en recursos provistos por doctores a en diversos idiomas en conjunto con el gran volumen de daño que ha causado denota la necesidad para hacer una excepción especial más alla de una referencia al pasar en Preguntas \ref{11-6}.

\textbf{Esta tabla es categóricamente falsa.} Casi todos los aspectos son engañosos en algún sentido, con la única excepcion de que es enteramente cierto que el estrógeno no genera cambios en la voz. Los rangos de tiempo en “expected onset” (momento previsto) es engañoso, y la idea de un límite de tiempo para el “maximum effect” (efecto máximo) es tan engañoso que bordea lo criminal. Una multitud de cambios no estan listados (como efectos mentales) o son erróneos (la tabla se contradice a si misma sobre disfunción erectil). No se hace ninguna considreación con respecto a la calidad del régimen.

\textbf{Los aspectos clave de la tabla requieren un matiz que será discutido a lo largo de la guía, por lo que la conclusión general es que los cambios suceden a diferentes ratios.} Esta tabla genera bajas espectativas, provee una imagen inexacta de lo que hace la TRH, y predispone a las personas trans al fallo limitando su entendimiento de hormonas. Por favor hagan caso omiso de la tabla. Continuemos ahora despues de este pequeño desvío.
