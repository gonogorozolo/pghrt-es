...
\author{\href{https://katea.gay/}{Katie Tightpussy} - Traducido por Jade y Gon}
\date{\today}
...
\textbf{Si te gustaría donar para apoyar este proyecto,} \href{https://cash.app/Katitties}{CashApp}, \href{https://ko-fi.com/katitties}{Ko-Fi}, y \href{https://account.venmo.com/u/katitties}{Venmo} sirven. ¡Lo apreciaría!
...
No. Esto es suficientemente importante para que no lo haya relegado a la sección \ref{MM} “MITOS Y MISC”. \textbf{Las dosis de estrógeno no pueden ser comparadas de forma directa entre tipos y forma de aplicación.} 1 mg. de una no equivale a 1 mg. de otra. Los diferentes tipos y formas tienen diferentes propiedades que afectan a cuánto estrógeno es absorbido por tu cuerpo (\textit{“biodisponibilidad”}), en qué ratio y su vida media resultante.

\subsection{¿Qué es la “vida media”?}

De forma simple, la \textit{vida media} de una sustancia es el tiempo que tarda en eliminarse la mitad de ella. En el contexto de la TRH, esto es lo que determina cuánto tiempo se mantiene una dosis activa en tu sistema, y por ende la frecuencia de dichas dosis. A esto nos referimos como ciclo hormonal, que forma una curva. Los niveles suben, llegan a una cima y luego caen. Las propiedades de esta curva (cómo los niveles de estrógeno cambian con el tiempo) son importantes.

\subsection{¿Qué tienen de malo las píldoras?}

El mayor problema de las píldoras es que, a largo plazo, incrementan el riesgo de coagulación sanguínea y coagulación de hígado. La severidad de estos riesgos puede ser mitigado, en parte, ingiríendolas de forma sublingual o bucal (disolviendo la píldora debajo de tu lengua o entre tus encías y mejilla, respectivamente) en contraposición a oralmente (tragándola de forma normal). Ésto evita la metabolización de primer paso en el hígado. Incluso con los métodos sublingual y bucal es común tragar alguna cantidad de la píldora, por lo que es posible asumir que existe algo de riesgo.
...
Más allá de esto, hay numerosos otros problemas de las píldoras debidos a dos características principales: 1) Su corta vida media y biodisponibilidad pobre, y 2) su necesidad común de antiandrógenos. La primera característica hace que las píldoras sean mayormente no aptas para monoterapia (discutido luego) comparadas con las inyecciones. La segunda viene acompañada por diversos efectos secundarios negativos, dependiendo de los antiandrógenos involucrados (ver sección \ref{AA} “ANTIANDRÓGENOS”).  Juntas, estas características suman grados adicionales de variabilidad que generan regímenes pobres, y sus efectos secundarios (como poca energía/libido y resultados más lentos) son más comunes que con otros tipos de administración. Las píldoras son además más dificiles de acumular, y, en algunos mercados, son más caras que los viales. Ten en cuenta además que importar píldoras de distribuidores extranjeros en grandes volúmenes puede hacer sonar las alarmas aduaneras, lo que puede generar confiscaciones, pérdida monetaria y/o problemas legales, dependiendo de las leyes del país en que residas. \textbf{Si alguien pregunta, no sabes quien pidió esas píldoras.}
...
\begin{itemize}
  \item Generalmente mucho más caros que cualquier otra opción;
  \item Pocos proveedores los ofrecen;
  \item Los periodos de ajuste de dosis están muy diseminados temporalmente;
...
Son bastante experimentales por lo que no hay mucho para decir de ellos, pero comparten los pros y contras del gel. Esta sección está principalmente para que puedas saber de su existencia (Nota de la traductora: En España se dejó de comercializar el spray Lenzetto a mediados de 2026, y no hay otros sprays de estradiol disponibles en el mercado español).
...
\subsection{¿Es precisa esta tabla?}\label{1-12}
...
El \textit{Valerato de Estradiol} es el éster más quisquilloso de los cuatro. Rápidamente alcanza un pico muy alto unos días después de la inyección y también cae de nuevo rápidamente. Esta relativa inestabilidad puede ser desagradable dependiendo de tus sensibilidades personales, pero con ajustes a la frecuencia y dosis esto se puede mitigar hasta cierto punto.

\begin{figure}[H]
  \centering
  \includegraphics[width=1\linewidth]{ev.png}
  \caption{Estradiol en suero (pg/ml) a partir de Valerato de Estradiol vs Tiempo (días)}
  \label{fig:ev}
\end{figure}
...
Esta es probablemente la pregunta más controvertida en el tema de la transición. La respuesta corta es que quieres lo suficiente como para sentirte bien y para suprimir la testosterona si lo necesitas, pero más allá de eso, niveles más altos son innecesariamente derrochadores en el mejor de los casos y pueden ser contraproducentes en el peor de los casos. Sin embargo, este es un rango amplio, y con tantas variables siempre hay una desviación personal. En otras palabras: Quieres suficiente estrógeno para sentirte bien, y eso es todo.
...
A menudo escucharás estos términos en el contexto de las inyecciones. \textit{Intramuscular} significa inyectado en el músculo y \textit{subcutánea} significa inyectado en la capa grasa debajo de tu piel.
...
Como nota al margen, los proveedores farmacéuticos de los viales de estradiol típicamente dicen que son solo para inyecciones intramusculares porque eso es para lo que están técnicamente aprobados, pero no hay una diferencia inherente. El contenido del vial a menudo es idéntico de todos modos, y el contenido del vial es lo que importa en última instancia. Sigue leyendo.
...
El \textit{calibre de la aguja} es una medida del grosor de la aguja. Cuanto mayor es el número, más delgada es la aguja. Por ejemplo, una aguja 25G es más delgada que una aguja 20G. Las agujas de mayor calibre también tienden a ser más cortas porque las agujas más largas se vuelven más propensas a doblarse, por lo que su longitud tiene un máximo inferior. No es sorprendente que las agujas más delgadas generalmente duelan menos. Cabe señalar que el calibre de la(s) aguja(s) utilizada(s) no afectará a la TRH de ninguna manera; solo afectará a la facilidad y comodidad de la inyección en sí.
...
La elección es tuya, pero a veces no hay elección. Hay pros y contras para cada uno. Por supuesto, no hay nada que te impida obtener estrógeno de múltiples proveedores para obtener los beneficios de ambos. En muchas situaciones, puede ser recomendable.
...
\begin{itemize}
  ...
  \item \textbf{Al menos aparentar estar usando proveedores farmacéuticos puede ser requerido si estás buscando la aprobación del seguro para cirugías.}
\end{itemize}
...
\end{itemize}
Adicionalmente, si estás buscando aprobación del seguro para cirugías, es probable que pidan una cantidad mínima de tiempo con una receta de TRH. Esto puede ser o no una preocupación para ti.
...
¿Qué eres? ¿Policía? No te lo voy a decir. Ese no es el objetivo de esta guía de todas formas. Hay otros recursos que tienen esa información. Céntrate.
...
Calienta tu sitio de inyección antes de inyectar. Relajar los músculos con un masaje o una ducha caliente antes de inyectar (específicamente, aumentar la temperatura con el agua apuntada hacia tu sitio de inyección antes de que salgas) puede ayudar.
...
\section{PROGESTERONA}

\subsection{¿Quiero tomar progesterona?}

\textbf{Probablemente.} Esta es una pregunta controvertida por alguna razón. Los detractores (específicamente, los médicos) argumentarán que no hay estudios que muestren que juega un papel en la feminización y por lo tanto no debería tomarse. Aparte de que las personas transfemeninas están lamentablemente poco estudiadas, heurísticamente hablando, la progesterona es una hormona sexual femenina clave que desempeña un papel importante en el cerebro y en muchas funciones a lo largo del cuerpo. Independientemente de los efectos de feminización física, es una hormona importante para la buena salud que no debe ser pasada por alto a la ligera.

\subsection{¿Cuál es la diferencia entre “progesterona”, “progestina” y “progestógeno”?}

La clase de hormonas, tanto naturales como sintéticas, que activan el receptor de progesterona son “proges\textbf{tógenos}”. El progestógeno natural, bioidéntico y más importante es la “proges\textbf{terona}”. Los progestógenos sintéticos son “proges\textbf{tinas}”. Estos tres términos se usan erróneamente de manera intercambiable en la literatura científica y en entornos clínicos, lo que probablemente cause gran parte de la confusión generalizada sobre el papel de la progesterona en la TRH, a pesar de que no son \textbf{equivalentes}.

\subsection{¿Quiero progesterona o una progestina?}

Progesterona. Quieres progesterona bioidéntica.

\subsection{¿Qué tienen de malo las progestinas?}

Las progestinas, más típicamente la \textit{medroxiprogesterona}, el \textit{acetato de medroxiprogesterona} o el \textit{levonorgestrel}, generalmente se asocian con los efectos secundarios negativos y riesgos a largo plazo (cáncer de mama, coágulos sanguíneos, depresión, etc.) que se atribuyen falsamente a la progesterona. No son bioidénticas, lo que significa que no se comportan igual que la progesterona y por lo tanto no pueden ser comparadas directamente.

\subsection{¿Qué hace la progesterona por la feminización?}

Se cree que la progesterona juega un papel en el desarrollo mamario y la libido en particular, pero como mencioné, es una hormona clave aparte de sus efectos sobre la apariencia externa. También tiene algunas propiedades antigonadotrópicas (es decir, contribuye a la supresión de la testosterona) que a veces pueden ser relevantes.

\subsection{¿Importa cuándo empiezo a tomar progesterona?}

Se desconoce. Hay cierta creencia de que comenzar demasiado temprano puede dañar el desarrollo mamario a largo plazo, pero esto es puramente teórico y la evidencia anecdótica contraria hace que la respuesta sea poco clara. La estimación conservadora es esperar aproximadamente un año después de comenzar la TRH (hasta el Estadio de Tanner 3 o 4) en el posible caso de que sí importe.

\subsection{¿Cómo se toma normalmente la progesterona?}

Aparte de las aplicaciones tópicas, la forma principal es a través de una píldora. Se prescribe como una píldora oral pero es más efectiva cuando se toma como un supositorio. Los sprays y cremas tópicas también pueden funcionar muy bien.

\subsection{¿Dices en serio que la progesterona debería ser tomada como un supositorio?}

La progesterona se metaboliza de manera completamente diferente cuando se toma a través de vía oral en vez de rectal debido a que pasa por el hígado cuando se toma por vía oral. La progesterona oral se convierte principalmente en \textit{alopregnanolona}, la cual puede causar mucha somnolencia, mientras que la progesterona administrada por vía rectal se convierte principalmente en la propia progesterona, que es lo que queremos (aunque algo todavía se convierte). Algunas personas toman adicionalmente progesterona también por vía oral como ayuda para dormir, pero ten en cuenta que demasiada \textit{alopregnanolona} a veces puede llevar a efectos secundarios negativos para la salud mental.

\subsection{¿Cómo tomo progesterona como un supositorio?}

Simplemente un poco de agua sobre la píldora debería funcionar, luego sécate y lávate las manos. Obviamente, no vayas al baño durante la próxima hora o así, así que ponértela antes de acostarte es lo mejor. Si estás teniendo problemas con que no se disuelva, entonces puedes intentar perforar la cápsula pero usualmente no debería haber ningún problema. Ten en cuenta que si usas supositorios caseros grandes hechos con aceite de coco, todo ese gran volumen de aceite de coco no querrá quedarse dentro de ti.

\subsection{¿Cuánta progesterona debería tomar?}

Para las píldoras, la dosis standard es de 100-200mg diarios por la noche. Es una dosis bastante arbitraria; 200mg es el máximo que la mayoría de los médicos prescribirá. Algunas personas toman más de 200mg en ocasiones, pero ten en cuenta que aumentar tus niveles puede llevar a un bajón desagradable. Ver la pregunta a continuación.

Para aplicaciones tópicas, nadie puede decírtelo con certeza debido a la alta variabilidad del medio de entrega, ni hay una guía clara sobre los niveles deseados, o incluso la frecuencia (probablemente diaria), ya que la progesterona simplemente está poco estudiada. Debido a esto, te aconsejaría ajustar tu dosis para que entiendas cómo te afecta la progesterona.

\subsection{¿Hay algún beneficio en "ciclar" la progesterona?}\label{8-11}

No. Algunas personas hacen esto para imitar el ciclo menstrual de una mujer cis, pero no hay razón para creer que haya algún beneficio por hacer esto y puede causar síntomas negativos de \textit{PMS} (síndrome premenstrual). La única excepción es si tienes buenas razones para sospechar que tienes una condición intersexual que involucre gestionar un útero. De lo contrario, lo desaconsejo. Ver Pregunta \ref{11-10}.

\subsection{¿Durante cuánto tiempo debería tomar progesterona?}

Durante el mismo tiempo que planees tomar estrógeno y durante el mismo tiempo que quieras. Así que probablemente para siempre.

A veces algunas personas (o los médicos) dicen arbitrariamente que solo se debe tomar progesterona durante X años. No hay ninguna razón teórica o empírica para sugerir que este sea un consejo sólido. Es más o menos tan coherente como si alguien (o un médico) preguntara cuánto tiempo planeas tomar TRH \textemdash{}oh, espera, no he dicho nada, claro que lo preguntan.

\subsection{¿Puede la progesterona convertirse en \textit{dihidrotestosterona} (DHT)?}

No. Bueno, estrictamente hablando, sí, pero también no. Es mayormente un mito, aunque \href{https://whsah.co/posts/rethinking-progesterone-and-androgens/}{como se detalla en este artículo de Alix}, para casos de personas con \textit{hiperplasia suprarrenal congénita no clásica (ncCAH)} la progesterona puede causar algunos efectos secundarios negativos de aumento de la actividad androgénica. En esos casos, se recomienda dejar de tomar progesterona y buscar un diagnóstico/tratamiento formal para potenciales trastornos suprarrenales.

\subsection{¿Hay algún beneficio en realizar aplicaciones tópicas de progesterona además de las píldoras?}

Quizá. Es una posible alternativa a las píldoras, especialmente en el caso de alguien con alergia al cacahuete ya que el fabricante de píldoras más común usa aceite de cacahuete, pero de nuevo la dosis es poco clara. Al menos algunas personas encuentran divertido tener más progesterona. Ten cuidado y diviértete.

Aclaración: Aplica las cremas en la región de tu muslo interno (u otra parte si se indica) u opcionalmente en la piel escrotal (es delgada y altamente vascularizada) en el caso de los sprays. Y no, aplicar progesterona directamente en tus senos es poco probable que los haga crecer más grandes o de forma más rápida en comparación con lo que haría de otra manera.

\subsection{¿Puedo esnifar polvo de progesterona?}

Por favor, no lo hagas. Es horrible para tus senos nasales. No es difícil hacer tu propio spray tópico de progesterona y hay guías por ahí. Haz eso en su lugar. Es significativamente más efectivo, consistente y seguro.

\subsection{¿Dónde puedo conseguir progesterona?}

La progesterona tiende a ser más cara a través de proveedores DIY debido a la mayor masa de hormonas requerida, así que idealmente consíguela a través de proveedores farmacéuticos cubiertos por el seguro. También existe la opción de farmacias extranjeras del mercado gris, que son simplemente farmacias en otro país, aunque a menudo requieren superar algunos obstáculos para comprar en ellas. En algunos lugares las cremas tópicas de progesterona están disponibles \textit{OTC} (\textit{Over The Counter}, de venta libre en farmacias), aunque no siempre es lo más económico dependiendo de la concentración.

\subsection{Me gustaría leer más sobre la progesterona dentro del contexto de la TRH. ¿Qué recursos debería leer?}\label{8-17}

Originalmente enlacé un documento aquí, pero opté por eliminarlo debido a una serie de defectos que pueden llevar a error. El problema con la progesterona es que literalmente nadie está de acuerdo sobre un solo aspecto de ella. No conozco una sola fuente o estudio en el que la gente esté de acuerdo en que es bueno. Demonios, ni siquiera están de acuerdo en si la palabra comienza con la letra “P”. \textbf{Lo crucial es saber que la progesterona no es estrictamente necesaria para una feminización completa o un buen desarrollo mamario, pero asumiendo que no está contraindicada para ti, probablemente valga la pena tomarla.}

Debería tenerse en cuenta que para toda la categoría de progestógenos hay innumerables mitos y falsedades inventados de la nada tanto por los defensores como por los detractores, lo que no hace que discernir la verdad de la ya deficiente erudición sea más fácil. Las afirmaciones fantásticas de beneficios mágicos y el alarmismo de riesgos supuestos basados en nada tampoco ayudan igualmente, aunque este último es peor en mi opinión cuando proviene de una autoridad médica, ya sea negligente o malintencionada.

\subsection{¿La progesterona interactúa con otros medicamentos relacionados con la TRH?}\label{8-18}

Si estás tomando inhibidores de la 5$\alpha$-reductasa como la \textit{Finasterida} y la \textit{Dutasterida} (ver Sección \ref{AA} “ANTIANDRÓGENOS”, o sigue leyendo), estos pueden afectar a cómo la progesterona se descompone naturalmente en \textit{Alopregnanolona}, lo cual puede causar efectos negativos en el estado de ánimo de algunas personas, independientemente de cómo estés tomando la progesterona. No está completamente clara la diferencia que supone la vía de administración para los inhibidores de la 5$\alpha$-reductasa (es decir, tópica contra oral), pero una menor absorción sistémica a través de la aplicación tópica puede mitigar estos efectos secundarios. Se recomienda no tomar ninguno de ellos si eres alguien afectado por esta interacción, pero no es en todos los casos de todos modos. Ten en cuenta que estos efectos depresivos pueden sentirse durante hasta un mes después de dejar de tomarlo.


\section{TESTOSTERONA}\label{T}

\subsection{¿Por qué no queremos cero testosterona?}

La testosterona es una hormona sexual esencial que desempeña un papel clave en tu salud y bienestar. Queremos suprimirla para la feminización, pero niveles cercanos a cero de testosterona (menos de 10 ng/dl, o 0.35 nmol/L) pueden causar problemas como baja libido, poca energía, baja fuerza (fatiga más allá de la mera pérdida de fuerza de la TRH), mala concentración, problemas para dormir, etc. Notablemente, son problemas muy similares a tener muy poco estrógeno. Las mujeres cis también tienen más que cero testosterona, así que eso no debería darnos miedo. \textbf{¡Los niveles hormonales adecuados son importantes!}

\subsection{¿Puede haber casos en los que querría suplementar la testosterona?}\label{9-2}

Sí. Si estás experimentando los problemas mencionados anteriormente y tus niveles de estrógeno son buenos, es posible que quieras suplementar con una microdosis de testosterona. Si deseas mejorar tu función eréctil, minimizar cualquier atrofia antes de la cirugía de reasignación de sexo, o simplemente experimentar con tus hormonas para ver qué se siente mejor para ti, entonces podría ser una razón para explorar la testosterona en un contexto diferente que puedas apreciar más en comparación con antes de la TRH.

\subsection{Si quisiera suplementar la testosterona, ¿cómo lo haría?}

Hay unas pocas posibilidades. La testosterona viene en inyecciones o en geles/cremas tópicas, de forma similar al estrógeno, como ya se discutió. Lo más probable es que te receten la forma tópica. Las aplicaciones tópicas tienen las desventajas que hemos discutido para el estrógeno, pero son menos preocupantes aquí ya quetener niveles precisos es menos importante.

\subsection{¿Cuáles son las formas tópicas de la testosterona?}

Hay gel y crema. El gel es lo que se recetará típicamente, pero algunas farmacias de las que ofrecen elaboraciones de fórmula magistral pueden hacer crema de baja penetración si alguien quisiera solo aplicación tópica en los genitales. Esta última es más difícil de conseguir y generalmente más cara, sin embargo.

\subsection{¿Importa dónde aplico la testosterona?}

Depende de si tienes gel o crema. Si tienes el tipo de crema local mencionado anteriormente, la aplicarías directamente como se indicó. De lo contrario, debería aplicarse el gel en los hombros y la parte superior de los brazos. ¡Asegúrate de no tocar cosas hasta mucho después de que se seque!

\subsection{¿Cuánta testosterona y con qué frecuencia debería aplicar?}

Adereza a tu gusto. Esto depende en gran medida de cómo te sientas. Si tienes demasiada, podrías empezar a experimentar los efectos secundarios de la testosterona (por ejemplo, piel grasa y vello corporal), pero solo tú puedes decir qué es lo que prefieres. Una inyección semanal de 5-10mg de \textit{cipionato de testosterona} podría funcionar para ti, pero en el caso de geles tópicos al 1\% que a menudo se distribuyen en paquetes de 25/50mg, hay más variabilidad. Casi nunca querrás ni siquiera la mitad de un paquete, y definitivamente no a diario. Sugeriría comenzar con mucho menos de lo que piensas para ver cómo te sientes.

\subsection{¿Dónde podría obtener testosterona?}

Si eres americano, tendrías que obtener una receta o preguntar a cualquier \textit{juicer} (consumidor de esteroides) en tu Planet Fitness más cercano. En otros lugares, depende de qué cadena de gimnasios esté más cerca de ti. Descargo de responsabilidad: Esto es una broma. Ver Pregunta \ref{6-11}.

\subsection{¿Hay otros esteroides equivalentes a la testosterona en el contexto de la TRH?}

Los esteroides anabólicos-androgénicos, es decir, drogas que son estructuralmente similares a la testosterona, no son completamente equivalentes. Los esteroides comúnmente usados en el mercado negro como el \textit{acetato de trembolona} tienen una larga lista de efectos secundarios no deseables, pero esteroides como el \textit{decanoato de nandrolona} se usan ocasionalmente para mujeres cis posmenopáusicas debido a sus propiedades androgénicas relativamente bajas, lo que los hace muy favorables para personas transfemeninas. De todos modos, en América es poco probable que te receten algo más allá de la propia testosterona, si es que puedes obtener una receta en absoluto.

\subsection{¿Cuál es la relación entre la testosterona y la \textit{dihidrotestosterona} (DHT)?}

La \textit{dihidrotestosterona} (DHT) es principalmente sintetizada a partir de la testosterona a través de la enzima 5$\alpha$-reductasa, convirtiéndose aproximadamente el 5\% de la testosterona de tu cuerpo. En general, si los niveles de testosterona están suprimidos (o si has tenido cirugía de reasignación) entonces no debería quedar mucho que convertir, pero los niveles sistémicos no serán cero porque aún se produce localmente. Dependiendo de tu cuerpo, esta sería la principal razón por la que podrías considerar suplementarte con un inhibidor antiandrogénico de la 5$\alpha$-reductasa tal como se discute en la siguiente sección. Como recordatorio, la \textit{dihidrotestosterona} es la hormona que es responsable del vello corporal y de la pérdida de cabello.

\textbf{Para cualquier persona trans masc leyendo esto}, me desviaré brevemente para señalar que en el momento de escribir esto no está claro qué papel juega la hormona en el crecimiento de los genitales en cuanto a velocidad o tamaño total en relación con la inhibición de la 5$\alpha$-reductasa. Es decir, se sabe que la \textit{dihidrotestosterona} juega un papel principal en el desarrollo del pene, pero no está claro cómo afecta directamente la falta de ésta a una persona trans masc. Aplicando el conocimiento del tratamiento de un micropene, sabemos que una crema tópica es más efectiva que las inyecciones exógenas, particularmente en cuanto a cómo la crema de \textit{dihidrotestosterona} es útil cuando un paciente no responde a la testosterona (particularmente en el caso de deficiencias de la 5$\alpha$-reductasa). Así que, algo para reflexionar. Que alguien traiga a Oliver Longdick para manejar el resto de este tema.
